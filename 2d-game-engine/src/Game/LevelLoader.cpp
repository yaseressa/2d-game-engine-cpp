...
LevelLoader::LevelLoader() {
    Logger::Info("LevelLoader constructor called!");
}

LevelLoader::~LevelLoader() {
    Logger::Info("LevelLoader destructor called!");
}

void LevelLoader::LoadLevel(sol::state& lua, int levelNumber, SDL_Renderer* renderer, const std::unique_ptr<WorldCordinator>&  worldCordinator, const std::unique_ptr<AssetManager>& assetManager) {
    ...
    while (true) {
        ...
        if (assetType == "texture") {
            ...
            Logger::Info("A new texture asset was added to the asset store, id: " + assetId);
        }
        if (assetType == "font") {
            assetManager->AddFont(assetId, asset["file"], asset["font_size"]);
            Logger::Info("A new font asset was added to the asset store, id: " + assetId);
        }
        i++;
    }
    ...
    for (int y = 0; y < mapNumRows; y++) {
        for (int x = 0; x < mapNumCols; x++) {
            ...
            tile.AddComponent<SpriteComponent>(mapTextureAssetId, tileSize, tileSize, 0, srcRectX, srcRectY, false);
        }
    }
    mapFile.close();
    ...
    while (true) {
        ...
        Entity newEntity = worldCordinator ->CreateEntity();
        ...
        if (hasComponents != sol::nullopt) {
            ...
            if (collider != sol::nullopt) {
                newEntity.AddComponent<BoxCollidorComponent>(
            ...
            if (keyboardControlled != sol::nullopt) {
                newEntity.AddComponent<KeyboardMovementComponent>(
                    glm::vec2(
